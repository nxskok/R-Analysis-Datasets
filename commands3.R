...
library(Kendall)
mk=apply(hbdata7,2,MannKendall)
mk
pval=matrix(0,37,3)
nmv=numeric(37)
pv=numeric(37)
...
for (i in 1:36)
{
  lmx=lm(hbdata7[,i+1]~hbdata7[,1])
  notmissing=!is.na(hbdata7[,i+1])
  v=hbdata7[notmissing,i+1]
  w=hbdata7[notmissing,1]
  tsx=mblm(v~w,repeated=F)
  slope.ts=tsx$coefficients[2]
  coeffs=summary(lmx)$coefficients
  slope.lm=coeffs[2,1]
  pvalue.lm=coeffs[2,4]
  row.i=c(slope.ts,slope.lm,pvalue.lm)
  results=rbind(results,row.i)
}

dimnames(results)[[1]]=1:36
dimnames(results)[[2]]=c("TS slope","Reg slope","Reg P-value")
