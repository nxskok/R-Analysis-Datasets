...
```{r}
fname="HudsonStrait_Brkup.csv"
fname_coords="Hudson Strait Location Coordinates 3 proper.xlsx"
...
mk_map(tab, coordinates, long_lim=c(63, 83), lat_lim=c(57, 66), longlegend=83, latlegend = 66)
```
...
draw_ts_map(locations, xlim=c(-80, -63), ylim=c(57, 66), slopes$theil_sen, scaling=2.7)
```
...
library(rworldmap)
library("readxl")
locations=read_excel("Hudson Strait Location Coordinates 3 proper.xlsx", skip=1)
lons=c(-80,-63)
lats=c(57,66)
```
...
```{r}
fname="HudsonStrait_Icefree.csv"
fname_coords="Hudson Strait Location Coordinates 3 proper.xlsx"
sea_ice_data=read_in(fname)
sea_ice_long=make_long(sea_ice_data)
```
